...
**Задача:**  
Дана матрица $4 \times 6$. Определить сумму элементов каждого столбца. Организовать ввод матрицы и вывод результатов.
...
function sumOfCols(matrix) {
  return matrix[0].map((_, i) => matrix.reduce((acc, row) => acc + row[i], 0));
}
```
...
В результате выполнения лабораторной работы была разработана программа, обрабатывающая матрицы
